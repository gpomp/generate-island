...
function processVertex (v, divide, noise, colors, i) {
  ...
  v.x += -0.00125 + Math.random() * 0.0025;
  v.y += -0.00125 + Math.random() * 0.0025;
...
function processRiver (vertices, noise) {
  let riverX = 0;
  let riverY = 0;
  let rY = 0;
  const tempV = new THREE.Vector2();
  const tempPos = new THREE.Vector2();

  while (riverX < 1) {
    let vertCopy = vertices.filter(v => {  
      tempPos.set(v.x, v.y);
      return tempPos.distanceTo(tempV.set(-0.5 + riverX, riverY * 0.5)) < 0.075;
    });

    vertCopy.forEach((v, i) => {
      tempPos.set(v.x, v.y);
      const d = 1 - tempPos.distanceTo(tempV.set(-0.5 + riverX, riverY * 0.5)) / 0.075;
      v.z += d * 8;
    });
    riverY += noise.simplex2(riverX * 70, rY * 70) * 0.1;
    rY += 0.01;
    console.log(riverX, riverY);
    riverX += 0.02;
  }
}
