...
module.exports = function (app, opts = {}) {
 ...
 /* const boatGeom = new THREE.BoxBufferGeometry(1, 2, 1);
  ...
  function processBoat (boat, i, dt) {
    ...
    let nextPosX = (oPosX + Math.cos(boat.userData.direction) * boat.userData.speed * 50) % 1;
    let nextPosY = (oPosY + Math.sin(boat.userData.direction) * boat.userData.speed * 50) % 1;
    ...
    if (n < 0.4 || nextPosX <= 0.00001 || nextPosY <= 0.00001 || nextPosX >= 0.9999 || nextPosX >= 0.9999) {
      boat.userData.direction = (boat.userData.direction + boat.userData.dir * 0.05) % (Math.PI * 2);

      nextPosX = (oPosX + Math.cos(boat.userData.direction) * boat.userData.speed * 200) % 1;
      nextPosY = (oPosY + Math.sin(boat.userData.direction) * boat.userData.speed * 200) % 1;
    } else {
      // boat.userData.dir *= -1;
    }
