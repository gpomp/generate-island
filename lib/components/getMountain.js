...
module.exports = function (app, opts = {}) {
  ...
  const tempPlane = new THREE.PlaneGeometry(1, 1, 120, 120);
  ...
  const uniforms = Object.assign({}, phongUniforms, {
    ...
    middleIntersect: { type: 'v3', value: new THREE.Vector3() }
  });
  ...
  return {
    ...
    update (dt) {
      /*if (mesh) {
        var intersects = app.raycaster.intersectObject(mesh);
        if (intersects.length > 0) {
          const intersect = intersects[0];
          const face = intersect.face;
          v1.set(positions[face.a], positions[face.b], positions[face.c]);
          uniforms.middleIntersect.value.copy(v1);
          console.log( uniforms.middleIntersect.value);
        }
      }*/
